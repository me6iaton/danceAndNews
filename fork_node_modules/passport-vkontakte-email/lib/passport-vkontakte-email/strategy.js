...
var util = require('util')
  ...
  , emptyEmailError = require('../errors/emptyEmailError')
  , notVkontakteOauthError = require('../errors/notVkontakteOauthError');
...
function Strategy(options, verify) {
  ...
  options.scopeSeparator = options.scopeSeparator || ',';
	...
	this.name = 'vkontakte-email';
	this._getEmailURL = options.getEmailURL || '/auth/vkontakte/addemail'
	this._emailField = options.emailField || 'email';
	this._emptyEmailMessage = options.emptyEmailMessage || 'empty Email'
	this._notVkontakteOauthMessage = options.notVkontakteOauthMessage || 'not Vkontakte Oauth'
}
...
Strategy.prototype.authenticate = function(req, options) {
  ...
  var getEmailURL = options.getEmailURL || this._getEmailURL;
  if (callbackURL) {
...
//          }
  ...
  } else if(req.originalUrl == getEmailURL) {
      var emailField = options.emailField || this._emailField;

      var email = lookup(req.body, emailField) || lookup(req.query, emailField)
      if (!email) {
       return self.fail(new emptyEmailError(options.emptyEmailMessage || this._emptyEmailMessage));
      }
      if(!req.session.vkontacte){
       return self.fail(new notVkontakteOauthError(options.notVkontakteOauthMessage || this._notVkontakteOauthMessage ));
      }
      var vkontacteReqInfo = req.session.vkontacte;
      vkontacteReqInfo.profile.email = email;
      function verified(err, user, info) {
        if (err) { return self.error(err); }
        if (!user) { return self.fail(info); }
        self.success(user, info);
      };
      self._verify(req,vkontacteReqInfo.accessToken
        , vkontacteReqInfo.refreshToken
        , vkontacteReqInfo.profile
        , verified
      );
      delete req.session.vkontacte
  } else {
    // NOTE: The module oauth (0.9.5), which is a dependency, automatically adds
